...
func (p *Provider) IndexField(ctx context.Context, obj client.Object, field string, extractValue client.IndexerFunc) error {
	return p.cluster.GetFieldIndexer().IndexField(ctx, obj, "cluster/"+field, func(obj client.Object) []string {
		clusterName := obj.GetNamespace()

		obj.SetNamespace("default")
		defer obj.SetNamespace(clusterName)

		keys := extractValue(obj)
		withCluster := make([]string, 0, len(keys)*2)
		for _, key := range keys {
			withCluster = append(withCluster, fmt.Sprintf("%s/%s", clusterName, key))
		}
		return withCluster
	})
}
